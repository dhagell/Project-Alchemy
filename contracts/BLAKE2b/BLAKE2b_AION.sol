...
contract BLAKE2b is GasTest, BLAKE2_Constants{
  ...
  function G(uint64[16] v, uint128 a, uint128 b, uint128 c, uint128 d, uint64 x, uint64 y) constant {
       ...
       //NON-Optimised mixing function for AION without inline assembly
		v[a] = (v[a] + v[b] + x) % uint64(2**64) ;
		v[d] = (v[d] ^ v[a]) >> 32;
		v[c] = (v[c] + v[d])     % uint64(2**64);
		v[b] = (v[b] ^ v[c]) >> 24;
		v[a] = (v[a] + v[b] + y) % uint64(2**64);
		v[d] = (v[d] ^ v[a]) >> 16;
		v[c] = (v[c] + v[d])     % uint64(2**64);
		v[b] = (v[b] ^ v[c]) >> 63;
    ...
    for(i = 0; i <16; i++){ //Operate 16 words at a time
      ...
      if(k == 0){
        b=ctx.b[i/4];  //Load relevant input into buffer
		 ...
		 uint64 cc = uint64(2) ** uint64(64*(3-k));
		//AION Extract relevent input from buffer
		mi = uint64(b /  cc   ) && uint64(0xFFFFFFFFFFFFFFFF);
		m[i] = getWords(mi);
